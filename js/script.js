...
const onSubmitFn = (event) => {
  ...
  const timer = new CountdownTimer({
    selector: "#timer-1",
    targetDate: new Date(`${event.currentTarget.elements.inputDate.value}`),
  });
  document.querySelector(".formDate").reset();
  timer.start();
  activeBtn = true;
};

document.querySelector(".formDate").addEventListener("submit", onSubmitFn);